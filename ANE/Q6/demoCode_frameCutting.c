...
{
	int8_t voltageData = frame;		//mesure de voltage depuis lsb a gouche sur 8 bits
	int8_t currentData = frame >> 8;	//mesure de courant avec decalage de 8 bits
	int8_t powerData = frame >> 16;
	int8_t resistorData = frame >> 24;

	//-- display the different frame value --//
	printf("-- valeur voltage : %d \n",voltageData ); // pour le print f "ce qui est afficher avec format %d", de ce que je dois print (voltageData)
